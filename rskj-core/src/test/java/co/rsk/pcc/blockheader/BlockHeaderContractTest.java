...
public class BlockHeaderContractTest {
    ...
    private CallTransaction.Function getRSKDifficultyFunction;
    ...
    public void setUp() {
        ...
        world = new World();
        contract = (co.rsk.pcc.blockheader.BlockHeaderContract) precompiledContracts.getContractForAddress(null, new DataWord(BLOCK_HEADER_CONTRACT_ADDRESS));
        ...
        getRSKDifficultyFunction = getContractFunction(contract, GetRSKDifficulty.class);
    ...
    public void getRSKDifficulty() {
        ...
        byte[] encodedResult = contract.execute(getRSKDifficultyFunction.encode(new BigInteger("0")));
        Object[] decodedResult = getRSKDifficultyFunction.decodeResult(encodedResult);
    ...
    public void getRSKDifficultyForBlockAtDepth1000() {
        ...
        byte[] encodedResult = contract.execute(getRSKDifficultyFunction.encode(new BigInteger("1000")));
        Object[] decodedResult = getRSKDifficultyFunction.decodeResult(encodedResult);
    ...
    private Block mineBlock(Block parent, RskAddress coinbase) {
        ...
        BtcTransaction bitcoinMergedMiningCoinbaseTransaction = MinerUtils.getBitcoinMergedMiningCoinbaseTransaction(bitcoinNetworkParameters, mergedMiningHash.getBytes());
        BtcBlock bitcoinMergedMiningBlock = MinerUtils.getBitcoinMergedMiningBlock(bitcoinNetworkParameters, bitcoinMergedMiningCoinbaseTransaction);

        BigInteger targetDifficulty = DifficultyUtils.difficultyToTarget(parent.getDifficulty());

        new BlockMiner(config).findNonce(bitcoinMergedMiningBlock, targetDifficulty);
    ...
    @Test
    public void hasGetRSKDifficulty() {
        assertHasMethod(GetRSKDifficulty.class, true);
    }
